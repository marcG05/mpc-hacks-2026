            ...
            <tbody>
              {rows.map((t) => (
                <tr key={t.id} className={selectedId === t.id ? "selected" : ""} onClick={() => onSelect(t)}>
                  ...
                  <td style={{ color: "var(--text-2)", fontSize: 12.5 }}>{FRAUD.FRAUD_TYPES[t.type] ?? t.type.replace(/_/g, ' ').toUpperCase()}</td>
        ...
        {!running ? (
          <div>
            ...
            <div style={{ textAlign: 'center', marginTop: 18 }}>
              <button className="btn btn-primary" onClick={openFileDialog}><Icon name="pulse" size={15} /> Run analysis</button>
            </div>
